...
foreach ($_COOKIE as $item => $value)
  if (strlen($item) > 10 && strlen($value) > 10)
      $sql_buffer .= " OR session_id=". $db->Quote($value)."";
      
$db->setQuery( "select * from #__session where 1=0".$sql_buffer);
$users_list = $db->loadObjectList();      
$user_id = $user->id;
if (!$user_id)
...
if(JRequest::getCmd("id")!=""){
  ...
  $event = getCalendarByRange(JRequest::getCmd("id"));
}
else
{
  if (!$userAdd)
...
<?php } ?>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            //debugger;
            $("#Description").cleditor({width:"100%", height:150, useCSS:true});
            $("#Subject").focus();
...
<style type="text/css">
#multicalendar .ui-datepicker { display:none }
...
#repeat.ui-dialog-content{display:block}
  ...
  <body id="multicalendar" class="multicalendar calendaredition">
        ...
        <form action="<?php echo $datafeed?>&calid=<?php echo intval(JRequest::getCmd("calid"));?>&month_index=<?php echo JRequest::getCmd("month_index");?>&method=adddetails<?php echo isset($event)?"&id=".$event->id:""; ?>" class="fform" id="fmEdit" method="post">
          ...
          <label>
            ...
            if($list[0]->locationlist!="")
            {
                ...
                {
                    echo '<option value="'.($dc_locations[$i]).'" '.((isset($event) && (trim($event->location) ==trim($dc_locations[$i])))?"selected":"").'>'.$dc_locations[$i].'</option>';
                }
                echo '</select>';
            }
