...
switch ($method) {
    case "add":
        ...
        break;
    case "list":
        if (JRequest::getVar("viewtype")=='list')
            $ret = listCalendarByPage($calid, JRequest::getVar("list_start"), JRequest::getVar("list_end"), JRequest::getVar("list_order"), JRequest::getVar("list_eventsPerPage"), JRequest::getVar("lastdate"));
        else
        {
...
function listCalendarByPage($calid, $list_start, $list_end, $list_order, $list_eventsPerPage, $lastdate){
  ...
  $db 	=JFactory::getDBO();
  try{
  $cond = DC_MV_CAL_IDCAL."=".$calid." and (rrule='' or rrule is null)";
  if ($list_start!="")
  {
      if ($list_order=="asc")
      {
    ...
    if (checkIfOverlapping(JRequest::getVar( 'calid' ), $st, $et,$sub,$loc,$id))
    {
        ...
        {           
            ...
            else
            {
                ...
                $cfrom = php2MySqlTime(js2PhpTime($st));
                $cto = php2MySqlTime(js2PhpTime($et));
                $cfrom = explode(" ", $cfrom);
                $cto = explode(" ", $cto);
                
                $sqldate =  " `".DC_MV_CAL_FROM."`='" . $starttime[0] . " " . $cfrom[1] . "', "
              . " `".DC_MV_CAL_TO."`='" . $endtime[0] . " " . $cto[1] . "', ";
            }
            $sql = "update `".DC_MV_CAL."` set " . $sqldate              
