...
class JFormFieldNMonths extends JFormField
{
	...
	protected function getInput()
	{
		...
		$html[] = '<div><input type="checkbox" name="'.$this->name.'0" id="'.$this->id.'0"' .
				' value="1" '.$checked1.' onclick="javascript:showhide(\''.$this->id.'\')"/><span style="padding:5px 5px 0px 0px">'.JText::_( 'SHOW TOOLTIP ON' ).'</span>' .
				...
				' value="1" '.$checked2.'/><span style="padding:5px 5px 0px 0px">'.JText::_( 'GO TO THE URL' ).'</span><input type="text" name="'.$this->name.'3" id="'.$this->id.'3"' .
				...
				'<select name="'.$this->name.'4" id="'.$this->id.'4"><option value="new_window" '.$selected3.' >'.JText::_( 'NEW WINDOW' ).'</option><option value="same_window" '.$selected4.'>'.JText::_( 'SAME WINDOW' ).'</option></select>' .
				'</div>';
