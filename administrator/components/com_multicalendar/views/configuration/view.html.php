...
class multicalendarViewconfiguration extends JViewLegacy
{
		...
		if (JRequest::getVar( 'task') == 'ajax')
		{
	        switch(JRequest::getVar( 'tab'))
		    {
		    	case '1':
		    	{
		    		...
		    		$value = unserialize ($rows[0]->administration);
		    		...
		    		$value["additional_parameters"] = JRequest::getVar('additional_parameters');
		    	...
		    	case '3':
		    	{
                    ...
                    $rows = $db->loadObjectList();
		    		$value = unserialize ($rows[0]->administration);
		    		$value["subjectlist"] = JRequest::getVar('subjectlist');
		    		$value["locationlist"] = JRequest::getVar('locationlist');
		    		$query = "update #__dc_mv_configuration set administration=".$db->Quote(serialize($value))." where id=1";
		    		$db->setQuery( $query );
		    		if (!$db->query())
		            {
