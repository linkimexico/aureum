...
class plgCommunityRseventspro extends CApplications
{
	...
	public function onProfileDisplay() {
		...
		$content	= $cache->call(array($this, 'getEventsHTML'), array($events, $booked, $user, $config));
		return $content; 
	}
	...
	public function getEventsHTML($rows , $booked , $user , $config) {
		...
		if ($rows) {
			...
			foreach ($rows as $eid) {
				...
				if ($event->allday)
					echo '<li><a '.$opener.' href="'.rseventsproHelper::route('index.php?option=com_rseventspro&layout=show&id='.rseventsproHelper::sef($event->id,$event->name),true,RseventsproHelperRoute::getEventsItemid()).'">'.$event->name.'</a> ('.rseventsproHelper::showdate($event->start,rseventsproHelper::getConfig('global_date'),true).')</li>';
				else 
					echo '<li><a '.$opener.' href="'.rseventsproHelper::route('index.php?option=com_rseventspro&layout=show&id='.rseventsproHelper::sef($event->id,$event->name),true,RseventsproHelperRoute::getEventsItemid()).'">'.$event->name.'</a> ('.rseventsproHelper::showdate($event->start,null,true).' - '.rseventsproHelper::showdate($event->end,null,true).')</li>';
			}
			echo '</ul>';
		}
