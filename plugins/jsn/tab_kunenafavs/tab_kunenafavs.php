...
class PlgJsnTab_Kunenafavs extends JPlugin
{
	public function renderTabs($data, $config)
	{
		...
		$plugin=array(JText::_($this->params->get('tabtitle','Forum Favorites')));
		...
		<script>
			function tabKunenafavsLoadMore(page)
			{
				
				jQuery("#tabKunenafavsLoadMore td").html("<img src=\"'.JURI::root().'media/plg_jsn_kunenafavs/loading.gif\" style=\"height:50px;\"/>");
				jQuery.get("index.php?option=com_jsn&view=loadmorekunenafavs&format=raw&uid='.$data->id.'&page="+page,function( data ){jQuery("#tabKunenafavsLoadMore").replaceWith(data);jQuery(".justLoaded").fadeIn(500).removeClass("justLoaded")});
	...
	{
		if(JRequest::getVar('view','')=='kunenaremovefav')
		{
			$db=JFactory::getDbo();
			$query='UPDATE #__kunena_user_topics SET favorite=0 WHERE user_id='.(int) JFactory::getUser()->id.' AND topic_id='.JRequest::getInt('tid');
			$db->setQuery($query);
			$db->execute();
		}
		
		if(JRequest::getInt('uid')!=JFactory::getUser()->id) return null;
		if(JRequest::getVar('view','')=='loadmorekunenafavs')
		{
			$lang = JFactory::getLanguage();
			$lang->load('com_content');
			KunenaFactory::loadLanguage('com_kunena.views', 'admin');
	...
	private function getPagination()
	{
		...
		$return=array();
		$return[]='<tr id="tabKunenafavsLoadMore" '.(JRequest::getVar('format','')=='raw' ? 'class="justLoaded"' : '').'><td style="text-align:center"><a class="btn btn-default" href="#" onclick="tabKunenafavsLoadMore('.$page.');return false;">'.JText::_('COM_CONTENT_MORE_ARTICLES').'</a></td></tr>';
		return implode('',$return);
	}
	...
	private function getRow($row)
	{
		...
		$return[]=' <a href="#" onclick="jQuery.get(\'index.php?option=com_jsn&view=kunenaremovefav&tid='.$row['id'].'&format=raw\');jQuery(this).closest(\'tr\').fadeOut(400,function(){jQuery(this).remove();});return false;" title="'.JText::_('COM_KUNENA_BUTTON_TOPIC_UNFAVORITE').'"><i class="icon icon-remove"></i></a>';
