...
<div class="box5">
	...
	<?php endif; ?>
	<form id="regForm">		
	<h1><?php echo __('Join')?> <?php echo $jsnsocial_setting['site_name']?></h1>
	<ul class="list1" id="regFields">
		<?php if ( $jsnsocial_setting['enable_registration_code'] ): ?>
    ...
    <div id="captcha" style="display:none">  
    	...
    	<div style="text-align: center">
	        <p><?php echo __('To avoid spam, please answer the following question')?>:</p><?php echo $challenge['SpamChallenge']['question']?><br />
	        <?php echo $this->Form->text('spam_challenge');?>
	    </div>
        <?php endif; ?>   
           
    	<?php if ( $jsnsocial_setting['recaptcha'] && !empty( $jsnsocial_setting['recaptcha_publickey'] ) && !empty( $jsnsocial_setting['recaptcha_privatekey'] ) ): ?>
    	<div class="captcha_box">
    	    <p><?php echo __('Please enter the two words in the box')?></p>
    	    <?php
            JsnApp::import('Vendor', 'recaptchalib');
            echo recaptcha_get_html( $jsnsocial_setting['recaptcha_publickey'] );
            ?>
    	</div>
    	<?php endif; ?>
