...
function copyFile()
{
	jQuery.fn.SimpleModal({
        ...
        callback: function(){
            ...
            jQuery.post('<?php echo $this->request->base?>/admin/themes/ajax_copy', { key: jQuery('#theme').val(), path: current_file }, function(data) {
                enableButton('proceedButton');
                
                if ( data == '' )
                    jsnsocialAlert('Template has been copied successfully');        
            }); 
        },
...
<div id="center">		
	...
	<div id="theme_info">
		<h2>Theme Info</h2>
		This is the base theme of JsnSocial. All themes inherits the template files from this theme. To override a template file, select it on the left column and copy it to your theme.
	</div>
	<?php else: ?>
	<div id="theme_info">
		<h2>Theme Info</h2>
