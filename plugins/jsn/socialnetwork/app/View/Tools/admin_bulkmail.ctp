...
function sendTestEmail()
{
	...
	jQuery.post("<?php echo $this->request->base?>/admin/tools/ajax_bulkmail_test", jQuery("#createForm").serialize(), function(data){
		enableButton('send_test_button');
		if (data != '') {
			jQuery(".error-message").show();
			jQuery(".error-message").html(data);
		} else {				
		    jsnsocialAlert('An email has been sent to <?php echo $jsnsocial_setting['site_email']?>');
		}
	});
}
...
function sendEmails()
{
	...
	jQuery.fn.SimpleModal({
        ...
        callback: function(){
            ...
            jQuery.post("<?php echo $this->request->base?>/admin/tools/ajax_bulkmail_start", jQuery("#createForm").serialize(), function(data){
