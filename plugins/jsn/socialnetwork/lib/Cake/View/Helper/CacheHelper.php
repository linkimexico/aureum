 ...
 */
class CacheHelper extends JsnAppHelper {
 ...
 */
	protected function _writeFile($content, $timestamp, $useCallbacks = false) {
		...
		if (empty($this->_View->plugin)) {
			$file .= "
			JsnApp::uses('{$this->_View->name}Controller', 'Controller');
			";
		} else {
			$file .= "
			JsnApp::uses('{$this->_View->plugin}AppController', '{$this->_View->plugin}.Controller');
			JsnApp::uses('{$this->_View->name}Controller', '{$this->_View->plugin}.Controller');
			";
		}
