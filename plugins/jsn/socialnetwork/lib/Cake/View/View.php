...
JsnApp::uses('HelperCollection', 'View');
JsnApp::uses('JsnAppHelper', 'View/Helper');
JsnApp::uses('Router', 'Routing');
JsnApp::uses('ViewBlock', 'View');
JsnApp::uses('CakeEvent', 'Event');
JsnApp::uses('CakeEventManager', 'Event');
JsnApp::uses('CakeResponse', 'Network');
 ...
 */
	protected function _getViewFileName($name = null) {
		...
		if ($this->plugin) {
			$pluginPaths = JsnApp::path('plugins');
			foreach ($paths as $path) {
 ...
 */
	protected function _paths($plugin = null, $cached = true) {
		...
		$paths = array();
		$viewPaths = JsnApp::path('View');
		$corePaths = array_merge(JsnApp::core('View'), JsnApp::core('Console/Templates/skel/View'));
		...
		if (!empty($plugin)) {
			...
			$paths = array_merge($paths, JsnApp::path('View', $plugin));
		}
