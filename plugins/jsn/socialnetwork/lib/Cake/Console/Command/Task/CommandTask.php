 ...
 */
	public function getShellList() {
		...
		$corePath = JsnApp::core('Console/Command');
		$shells = JsnApp::objects('file', $corePath[0]);
		$shells = array_diff($shells, $skipFiles);
		$this->_appendShells('CORE', $shells, $shellList);

		$appShells = JsnApp::objects('Console/Command', null, false);
		$appShells = array_diff($appShells, $shells, $skipFiles);
		$this->_appendShells('app', $appShells, $shellList);

		foreach ($plugins as $plugin) {
			$pluginShells = JsnApp::objects($plugin . '.Console/Command');
 ...
 */
	public function getShell($commandName) {
		...
		$class = $name . 'Shell';
		JsnApp::uses($class, $pluginDot . 'Console/Command');
