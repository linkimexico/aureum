 ...
 */
	public function execute() {
		...
		if (!empty($this->params['exclude-plugins']) && $this->_isExtractingApp()) {
			$this->_exclude = array_merge($this->_exclude, JsnApp::path('plugins'));
		}
 ...
 */
	protected function _extractValidationMessages() {
		...
		if (empty($this->params['exclude-plugins'])) {
			$plugins = array_merge($plugins, JsnApp::objects('plugin', null, false));
		}
 ...
 */
	protected function _extractPluginValidationMessages($plugin = null) {
		JsnApp::uses('AppModel', 'Model');
		if (!empty($plugin)) {
			if (!CakePlugin::loaded($plugin)) {
				return;
			}
			JsnApp::uses($plugin . 'AppModel', $plugin . '.Model');
			$plugin = $plugin . '.';
		}
		$models = JsnApp::objects($plugin . 'Model', null, false);

		foreach ($models as $model) {
			JsnApp::uses($model, $plugin . 'Model');
