...
JsnApp::uses('BakeTask', 'Console/Command/Task');
 ...
 * This is used to generate JsnApp::uses() call to autoload base
 ...
 */
	public function bake($type, $className) {
		...
		JsnApp::uses($baseClass, $baseType);
		JsnApp::uses($fullClassName, $realType);
 ...
 */
	public function getClassName($objectType) {
		...
		if ($this->plugin) {
			$plugin = $this->plugin . '.';
			$options = JsnApp::objects($plugin . $type);
		} else {
			$options = JsnApp::objects($type);
		}
 ...
 */
	public function isLoadableClass($package, $class) {
		JsnApp::uses($class, $package);
		list($plugin, $ns) = pluginSplit($package);
		if ($plugin) {
			JsnApp::uses("{$plugin}AppController", $package);
			JsnApp::uses("{$plugin}AppModel", $package);
			JsnApp::uses("{$plugin}JsnAppHelper", $package);
		}
...
/**
 * Map the types that TestTask uses to concrete types that JsnApp::uses can use.
 *
