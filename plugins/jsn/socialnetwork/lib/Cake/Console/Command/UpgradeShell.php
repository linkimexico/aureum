...
JsnApp::uses('AppShell', 'Console/Command');
 ...
 */
	public function tests() {
		...
		if (!empty($this->params['plugin'])) {
			$this->_paths = array(JsnApp::pluginPath($this->params['plugin']) . 'tests' . DS);
		}
		$patterns = array(
			array(
 ...
 */
	public function locations() {
		...
		if (!empty($this->params['plugin'])) {
			chdir(JsnApp::pluginPath($this->params['plugin']));
		}
 ...
 */
	public function helpers() {
		$this->_paths = array_diff(JsnApp::path('views'), JsnApp::core('views'));

		if (!empty($this->params['plugin'])) {
			$this->_paths = array(JsnApp::pluginPath($this->params['plugin']) . 'views' . DS);
		}

		$patterns = array();
		JsnApp::build(array(
			'View/Helper' => JsnApp::core('View/Helper'),
		), JsnApp::APPEND);
		$helpers = JsnApp::objects('helper');
		$plugins = JsnApp::objects('plugin');
		$pluginHelpers = array();
		foreach ($plugins as $plugin) {
			CakePlugin::load($plugin);
			$pluginHelpers = array_merge(
				$pluginHelpers,
				JsnApp::objects('helper', JsnApp::pluginPath($plugin) . DS . 'views' . DS . 'helpers' . DS, false)
			);
		}
 ...
 */
	public function i18n() {
		...
		if (!empty($this->params['plugin'])) {
			$this->_paths = array(JsnApp::pluginPath($this->params['plugin']));
		}
 ...
 */
	public function request() {
		$views = array_diff(JsnApp::path('views'), JsnApp::core('views'));
		$controllers = array_diff(JsnApp::path('controllers'), JsnApp::core('controllers'), array(APP));
		$components = array_diff(JsnApp::path('components'), JsnApp::core('components'));

		$this->_paths = array_merge($views, $controllers, $components);

		if (!empty($this->params['plugin'])) {
			$pluginPath = JsnApp::pluginPath($this->params['plugin']);
			$this->_paths = array(
 ...
 */
	public function controller_redirects() {
		$this->_paths = JsnApp::Path('Controller');
		if (!empty($this->params['plugin'])) {
			$this->_paths = JsnApp::Path('Controller', $this->params['plugin']);
		}
		$patterns = array(
			array(
 ...
 */
	public function components() {
		$this->_paths = JsnApp::Path('Controller/Component');
		if (!empty($this->params['plugin'])) {
			$this->_paths = JsnApp::Path('Controller/Component', $this->params['plugin']);
		}
		$patterns = array(
			array(
