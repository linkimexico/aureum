 ...
 * ### Adding paths
 *
 * You can add paths to the search indexes App uses to find classes using `JsnApp::build()`. Adding
 ...
 * classes reside. You can configure each package location in your application using `JsnApp::build('APackage/SubPackage', $paths)`
 ...
 *
 * You can inspect the currently loaded paths using `JsnApp::path('Controller')` for example to see loaded
 * controller paths.
 *
 * It is also possible to inspect paths for plugin classes, for instance, to see a plugin's helpers you would call
 * `JsnApp::path('View/Helper', 'MyPlugin')`
 *
 * ### Locating plugins and themes
 *
 * Plugins and Themes can be located with App as well. Using JsnApp::pluginPath('DebugKit') for example, will
 * give you the full path to the DebugKit plugin. JsnApp::themePath('purple'), would give the full path to the
 * `purple` theme.
 *
 * ### Inspecting known objects
 *
 * You can find out which objects App knows about using JsnApp::objects('Controller') for example to find
 ...
 */
	public static $types = array(
		...
		'helper' => array('suffix' => 'Helper', 'extends' => 'JsnAppHelper', 'core' => true),
 ...
 * Usage:
 *
 * `JsnApp::path('Model'); will return all paths for models`
 *
 * `JsnApp::path('Model/Datasource', 'MyPlugin'); will return the path for datasources under the 'MyPlugin' plugin`
 *
 * @param string $type type of path
 * @param string $plugin name of plugin
 * @return array
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/app.html#JsnApp::path
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/app.html#JsnApp::paths
 */
 ...
 * Usage:
 *
 * `JsnApp::build(array(Model' => array('/a/full/path/to/models/'))); will setup a new search path for the Model package`
 *
 * `JsnApp::build(array('Model' => array('/path/to/models/')), JsnApp::RESET); will setup the path as the only valid path for searching models`
 *
 * `JsnApp::build(array('View/Helper' => array('/path/to/helpers/', '/another/path/'))); will setup multiple search paths for helpers`
 *
 * `JsnApp::build(array('Service' => array('%s' . 'Service' . DS)), JsnApp::REGISTER); will register new package 'Service'`
 *
 ...
 * @param boolean|string $mode JsnApp::RESET will set paths, JsnApp::APPEND with append paths, JsnApp::PREPEND will prepend paths (default)
 * 	JsnApp::REGISTER will register new packages and their paths, %s in path will be replaced by APP path
 * @return void
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/app.html#JsnApp::build
 */
	public static function build($paths = array(), $mode = JsnApp::PREPEND) {
 ...
 * Usage:
 *
 * `JsnApp::pluginPath('MyPlugin'); will return the full path to 'MyPlugin' plugin'`
 *
 * @param string $plugin CamelCased/lower_cased plugin name to find the path of.
 * @return string full path to the plugin.
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/app.html#JsnApp::pluginPath
 */
 ...
 * Usage:
 *
 * `JsnApp::themePath('MyTheme'); will return the full path to the 'MyTheme' theme`
 *
 * @param string $theme theme name to find the path of.
 * @return string full path to the theme.
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/app.html#JsnApp::themePath
 */
 ...
 * Usage:
 *
 * `JsnApp::core('Cache/Engine'); will return the full path to the cache engines package`
 *
 * @param string $type
 * @return array full path to package
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/app.html#JsnApp::core
 */
 ...
 * Example usage:
 *
 * `JsnApp::objects('plugin');` returns `array('DebugKit', 'Blog', 'User');`
 *
 * `JsnApp::objects('Controller');` returns `array('PagesController', 'BlogController');`
 *
 * You can also search only within a plugin's objects by using the plugin dot
 * syntax.
 *
 * `JsnApp::objects('MyPlugin.Model');` returns `array('MyPluginPost', 'MyPluginComment');`
 *
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/app.html#JsnApp::objects
 */
 ...
 * Usage:
 *
 * `JsnApp::uses('MyCustomController', 'Controller');` will setup the class to be found under Controller package
 *
 * `JsnApp::uses('MyHelper', 'MyPlugin.View/Helper');` will setup the helper class to be found in plugin's helper package
 *
 * @param string $className the name of the class to configure package for
 * @param string $location the package name
 * @return void
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/app.html#JsnApp::uses
 */
 ...
 * defined using JsnApp::uses() and with this information it will resolve the package name to a full path
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/app.html#JsnApp::location
 */
...
/**
 * Finds classes based on $name or specific file(s) to search. Calling JsnApp::import() will
	...
	public static function import($type = null, $name = null, $parent = true, $search = array(), $file = null, $return = false) {
		...
		if (is_array($name)) {
			foreach ($name as $class) {
				if (!JsnApp::import(compact('type', 'parent', 'search', 'file', 'return') + array('name' => $class))) {
					return false;
				}
			}
			return true;
 ...
 * This is a compatibility wrapper around using JsnApp::uses() and automatic class loading
 *
 ...
 */
	protected static function _loadClass($name, $plugin, $type, $originalType, $parent) {
		...
		if ($parent && isset(self::$types[$originalType]['extends'])) {
			...
			JsnApp::uses($extends, $extendType);
			if ($plugin && in_array($originalType, array('controller', 'model'))) {
				JsnApp::uses($plugin . $extends, $plugin . '.' . $type);
			}
		}
		if ($plugin) {
			$plugin .= '.';
 ...
 */
	protected static function _loadVendor($name, $plugin, $file, $ext) {
		...
		$fileTries = array();
		$paths = ($plugin) ? JsnApp::path('vendors', $plugin) : JsnApp::path('vendors');
		if (empty($ext)) {
			$ext = 'php';
		}
		if (empty($file)) {
 ...
 */
	public static function init() {
		...
		/* JSN - START */
		if(isset(self::$_map['ErrorHandler']) && !file_exists(self::$_map['ErrorHandler'])){
			//echo(substr(CACHE,0,-1));
			self::_rrmdir(substr(CACHE,0,-1));
			die('Cache updated, please reload this page');
		}
		/* JSN - END */
		
		/* JSN - START */
		register_shutdown_function(array('JsnApp', 'shutdown'));
		/* JSN - END */
	}
