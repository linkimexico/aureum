 ...
 */
	public function __construct($url = null, $parseEnvironment = true) {
		...
		else
		{
			...
			{
				$itemid=$profileMenu->id;
			}
			else{
				$itemid=$menu->getDefault()->id;
			}
		}
		$this->base=str_replace('.html','',JRoute::_('index.php?option=com_jsn&view=social&Itemid='.$itemid.'&route=',false));
		$this->webroot=JURI::root(true).'/media/socialnetwork/';
		if(JFactory::getApplication()->getCfg('sef')=='0')
		{
			$this->base=str_replace('&route=','',$this->base).'&route=';
		}
		else
		{
			if(substr($this->base,-1)=='/') $this->base=substr($this->base,0,-1);
			if($this->base==JURI::root(true) && JFactory::getApplication()->getCfg('sef_rewrite')=='0')
			{
				$this->base=JURI::root(true)."/index.php";
			}
		}
		/* JSN - END */
		...
		/* JSN - START */
		$this->url=JRequest::getVar('route','');//$this->url = $url;
		/* JSN - END */
