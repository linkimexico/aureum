 ...
 * @package       Cake.Model
 */
class CakeSchema extends JsnObject {
 ...
 */
	public function read($options = array()) {
		...
		if (isset($this->plugin)) {
			JsnApp::uses($this->plugin . 'AppModel', $this->plugin . '.Model');
		}
		...
		if (!is_array($models) && $models !== false) {
			if (isset($this->plugin)) {
				$models = JsnApp::objects($this->plugin . '.Model', null, false);
			} else {
				$models = JsnApp::objects('Model');
			}
		}
		...
		if (is_array($models)) {
			foreach ($models as $model) {
				...
				JsnApp::uses($importModel, $plugin . 'Model');
