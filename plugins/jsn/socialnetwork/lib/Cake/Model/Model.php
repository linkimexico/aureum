...
JsnApp::uses('ClassRegistry', 'Utility');
JsnApp::uses('Validation', 'Utility');
JsnApp::uses('CakeString', 'Utility');
JsnApp::uses('Hash', 'Utility');
JsnApp::uses('BehaviorCollection', 'Model');
JsnApp::uses('ModelBehavior', 'Model');
JsnApp::uses('ModelValidator', 'Model');
JsnApp::uses('ConnectionManager', 'Model');
JsnApp::uses('Xml', 'Utility');
JsnApp::uses('CakeEvent', 'Event');
JsnApp::uses('CakeEventListener', 'Event');
JsnApp::uses('CakeEventManager', 'Event');
 ...
 */
class Model extends JsnObject implements CakeEventListener {
 ...
 */
	public function beforeFind($queryData) {
		/* JSN - START */
		$common=array('Activity','FriendRequest','EventRsvp','GroupUser','Blog','Photo','Group','Event','Video','Topic','Comment','Like','Album');
		if(in_array($this->name,$common))
		{
			$queryData['joins'] = array(
			    array('table' => 'joomla_users',
			        'alias' => 'Profile',
			        'type' => 'LEFT',
			        'conditions' => array(
			            'Profile.id = '.$this->name.'.user_id',
			        )
			    )
			);
			$conditions=array();
			$queryData['conditions']['Profile.id > ?']=0;
			foreach($queryData['conditions'] as $key => $value){
				if(strpos(' '.$key,'User.active')>0) $conditions[str_replace('User.active','Profile.block',$key)]=($value ? 0 : 1);
				elseif(strpos(' '.$key,'active')>0) $conditions[str_replace('active','Profile.block',$key)]=($value ? 0 : 1);
				elseif(strpos(' '.$key,'User.name')>0) $conditions[str_replace('User.name','Profile.name',$key)]=$value;
				else $conditions[$key]=$value;
			}
			$queryData['conditions']=$conditions;
			if(is_string($queryData['order'])) $queryData['order']=str_replace('User.name','Profile.name',$queryData['order']);
		}
		if($this->name=='Friend')
		{
			$queryData['joins'] = array(
			    array('table' => 'joomla_users',
			        ...
			        'type' => 'LEFT',
			        'conditions' => array(
			            'Profile.id = '.$this->name.'.friend_id',
			        )
			    )
			);
			$conditions=array();
			...
			{
				foreach($queryData['order'] as &$value){
					$value=str_replace('User.id','Profile.id',$value);
					$value=str_replace('User.name','Profile.name',$value);
				}
			}
		}
		if($this->name=='User')
		{
			$queryData['joins'] = array(
				array('table' => 'joomla_user_usergroup_map',
			        'alias' => 'Usergroups',
			        'type' => 'RIGHT',
			        'conditions' => array(
			...
			if(!(is_string($queryData['fields']) && strpos(' '.$queryData['fields'],'COUNT(*)')>0))
			{
				$queryData['fields']=array();
				$queryData['fields'][]= 'User.*';
				$queryData['fields'][]= 'Role.*';
				$queryData['fields'][]= 'Profile.id';
			}
			...
			if(isset($queryData['conditions']))
			{
				foreach($queryData['conditions'] as $key => $value){
					...
					elseif(strpos(' '.$key,'active')>0) $conditions[str_replace('active','Profile.block',$key)]=($value ? 0 : 1);
					elseif(strpos(' '.$key,'User.name')>0) $conditions[str_replace('User.name','Profile.name',$key)]=$value;
					elseif(strpos(' '.$key,'User.id')>0) $conditions[str_replace('User.id','Profile.id',$key)]=$value;
					else $conditions[$key]=$value;
				}
				$queryData['conditions']=$conditions;
			}
