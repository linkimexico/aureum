...
class SppagebuilderAddonTestimonialproadvance extends SppagebuilderAddons {

    public function render() {

        $settings = $this->addon->settings;
        $class = (isset($settings->class) && $settings->class) ? $settings->class : '';
        $slider_items = (isset($settings->slider_items) && $settings->slider_items) ? $settings->slider_items : 3;
        ...
        //Output
        $output = '<div id="sppb-testimonial-pro-' . $this->addon->id . '" class="sppb-testimonial-pro sppb-slide '.$content_alignment.' ' . $class . '">';

        if($show_quote){
            $output .= '<span class="fa fa-quote-left" aria-hidden="true"></span>';
        }
        $output .= '<div id="advance-testimonial" class="sppb-carousel-inner owl-carousel" data-slider-items="' . $slider_items . '">';

        foreach ($settings->sp_testimonialpro_item as $key => $value) {
            $output .= '<div class="sppb-item ' . (($key == 0) ? ' active' : '') . '">';
            $output .= '<div class="sppb-item-wrapper">';
    ...
    public function scripts(){
        ...
        $base_url = JURI::base() . '/templates/' . $app->getTemplate() . '/js/';
        return array($base_url . 'owl.carousel.min.js');
    }
    ...
    public function js(){
        ...
        return 'jQuery(document).ready(function ($) {
            ...
            var $advanceTestimonial = $("#advance-testimonial");
            var $sliderTtems = $advanceTestimonial.attr("data-slider-items");

            $advanceTestimonial.owlCarousel({
                margin: 120,
                loop: true,
                video: true,
                center: true,
    ...
    public function css() {
        ...
        $css .= $addon_id . ' .sppb-item .sppb-item-wrapper> img{width:'.$avatar_size.'px; height:'.$avatar_size.'px;}';
        //Icon Style
        $icon_style = '';
        $icon_style_sm = '';
        $icon_style_xs = '';
        ...
        if($icon_style_sm || $content_fontsize_sm || $arrow_margin_sm || $content_margin_sm || $name_font_size_sm || $name_line_height_sm || $content_lineheight_sm || $avatar_width_sm || $designation_style_sm ){
            $css .= '@media (min-width: 768px) and (max-width: 991px) {';
                ...
                if($avatar_width_sm){
                    $css .= $addon_id . ' .sppb-item .sppb-item-wrapper> img{width:'.$avatar_width_sm.'px; height:'.$avatar_width_sm.'px;}';
                }
        ...
        if($icon_style_xs || $content_fontsize_xs || $arrow_margin_xs || $content_margin_xs || $name_font_size_xs || $name_line_height_xs || $content_lineheight_xs || $avatar_width_xs || $designation_style_xs){
            $css .= '@media (max-width: 767px) {';
                ...
                if($avatar_width_xs){
                    $css .= $addon_id . ' .sppb-item .sppb-item-wrapper> img{width:'.$avatar_width_xs.'px; height:'.$avatar_width_xs.'px;}';
                }
    ...
    public static function getTemplate() {
        ...
        $output = '
            ...
            <style type="text/css">
                ...
                #sppb-addon-{{ data.id }} .sppb-item .sppb-item-wrapper> img,
            ...
            <div id="sppb-testimonial-pro-{{ data.id }}" class="sppb-testimonial-pro sppb-slide {{data.content_alignment}} {{ data.class }}">


                <# if(data.show_quote){ #>
                    <span class="fa fa-quote-left"></span>
                <# } #>
                <div id="advance-testimonial" class="sppb-carousel-inner owl-carousel" data-slider-items="{{data.slider_items}}">
                    <#
